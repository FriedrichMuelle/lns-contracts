...
abstract contract SmartBchAddrResolver is IAddrResolver, IAddressResolver, ResolverBase {
    uint constant private COIN_TYPE_SMARTBCH = 2147493648;
    ...
    function setAddr(bytes32 node, address a) virtual external authorised(node) {
        setAddr(node, COIN_TYPE_SMARTBCH, addressToBytes(a));
    }
    ...
    function addr(bytes32 node) virtual override public view returns (address payable) {
        bytes memory a = addr(node, COIN_TYPE_SMARTBCH);
        if(a.length == 0) {
            return payable(0);
        }
    ...
    function setAddr(bytes32 node, uint coinType, bytes memory a) virtual public authorised(node) {
        if(coinType == COIN_TYPE_SMARTBCH) {
