...
contract('NFTNativePriceOracle', function (accounts) {
    ...
    //     assert.equal(await priceOracle.price("test", 0, yearInSeconds), bnEth);
    //     assert.equal(await priceOracle.getBasePrice("test", 0, yearInSeconds), bnEth);
    //     assert.equal(await priceOracle.getDiscountedPrice(bnEth), bnEth);
    //     const breakdown = await priceOracle.getNamePriceBreakdown("test", 0, yearInSeconds);
    ...
    //     assert.equal(await priceOracle.price("test", 0, yearInSeconds), toBN(bnEth).mul(toBN(950)).div(toBN(1000)).toString());

    //     await NFT.mint(ownerAccount);
    //     assert.equal(await priceOracle.price("test", 0, yearInSeconds), toBN(bnEth).mul(toBN(900)).div(toBN(1000)).toString());

    //     await NFT.mint(ownerAccount);
    //     assert.equal(await priceOracle.price("test", 0, yearInSeconds), toBN(bnEth).mul(toBN(800)).div(toBN(1000)).toString());
    ...
    //     assert.equal((await priceOracle.price("test", 0, yearInSeconds)).toString(), toBN(bnEth).mul(toBN(950)).div(toBN(1000)).toString());

    //     await priceOracle.setRequiredNFTs(2);
    //     await exceptions.expectFailure(priceOracle.price("test", 0, yearInSeconds));

    //     await NFT.mint(ownerAccount);
    //     await exceptions.expectSuccess(priceOracle.price("test", 0, yearInSeconds));
    //     assert.equal((await priceOracle.price("test", 0, yearInSeconds)).toString(), toBN(bnEth).mul(toBN(900)).div(toBN(1000)).toString());
    // });
    ...
    //     await exceptions.expectFailure(priceOracle.price("newname3", 0, yearInSeconds));
    ...
    //     assert.equal(await priceOracle.supportsInterface("0xf3a79283"), true); // ORACLE_ID
    //     assert.equal(await priceOracle.supportsInterface("0xf67153a1"), true); // NFT_ORACLE_ID
    // });
    ...
    //     breakdown = await priceOracle.getNamePriceBreakdown("test", 0, yearInSeconds);
    //     assert.equal(breakdown.canRegisterName, true);
    //     assert.equal(breakdown.isEligibleForDiscount, false);
    //     assert.equal(breakdown.nftAmount, 0);
    //     assert.equal(breakdown.requiredNftAmount, 0);
    ...
    //     assert.equal(await priceOracle.countNFTs(), 0);
    //     await priceOracle.addEligibleCollection(NFT.address);

    //     await NFT.mint(ownerAccount);
    //     assert.equal(await priceOracle.countNFTs(), 1);
    ...
    it('should handle static calls', async () => {
        // delete priceOracle.abi
        // delete priceOracle.deployedBytecode
        // delete priceOracle.bytecode
        // delete priceOracle.contract
        // console.log(priceOracle)
        // priceOracle = await ethers.getContract("NFTNativePriceOracle")
        // console.log(priceOracle.contract.currentProvider)
        // assert.equal(await priceOracle.countNFTs(), 0);
        // priceOracle = priceOracle.contract.connect(priceOracle.contract.currentProvider)

        // console.log(priceOracle.signer)

        assert.equal(await priceOracle.countNFTs(), 0);
        assert.equal(await priceOracle.countNFTs({from: NULL_ADDRESS}), 0);
        assert.equal(await priceOracle.countNFTs({from: someone}), 0);

        assert.equal(await priceOracle.price("test", 0, yearInSeconds), bnEth);
        assert.equal(await priceOracle.price("test", 0, yearInSeconds, {from: NULL_ADDRESS}), bnEth);
        assert.equal(await priceOracle.price("test", 0, yearInSeconds, {from: someone}), bnEth);
